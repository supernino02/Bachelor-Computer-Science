...
public class MyLangParser implements Parser {
	...
	private Stmt parseStmt() throws ParserException {
		...
		case IF -> parseIfStmt();
		case FOR_EACH -> parseForEachStmt(); //new case
		default -> unexpectedTokenError();
		};
	}

	//new function used to parse
	private ForEachStmt parseForEachStmt() throws ParserException {
		consume(FOR_EACH);                 //'foreach'
		final var ident = parseVariable(); //IDENT
		consume(IN);                       //'in'
		final var vector = parseExp();     //Exp
		final var block = parseBlock();    //Block
		return new ForEachStmt(ident, vector, block);
	}
	// -----------------------------------
	// Updated code
	// -----------------------------------
	...
	private Exp parseAtom() throws ParserException {
		...
		case FST -> parseFst();
		case SND -> parseSnd();
		case OPEN_VEC -> parseVector(); //new case
		default -> unexpectedTokenError();
		};
	}

	//new case
	private VectorNode parseVector () throws ParserException {
		consume (OPEN_VEC);         //'['
		final var pos = parseExp(); //Exp
		consume (STMT_SEP);         //';'
		final var size = parseExp();//Exp
		consume (CLOSE_VEC);        //']'
		return new VectorNode (pos, size);
	}
	// -----------------------------------
	// Updated code
	// -----------------------------------
